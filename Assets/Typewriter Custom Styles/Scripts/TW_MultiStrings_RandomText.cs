...
public class TW_MultiStrings_RandomText : MonoBehaviour {
    ...
    private IEnumerator MakeRandomText(string ORIGINAL)
    {
        ...
        if (сharIndex != ORIGINAL.Length + 1)
        {
            ...
            string randomString = new string(Enumerable.Repeat(chars, ORIGINAL.Length+10).Select(s => s[random.Next(s.Length)]).ToArray());
    ...
    private IEnumerator MakeRandomTextWithNewLine(string ORIGINAL, List<int> List)
    {
        ...
        if (сharIndex != ORIGINAL.Length + 1)
        {
            ...
            string TEXT = randomString.Substring(0, сharIndex) + ORIGINAL.Substring(сharIndex);
